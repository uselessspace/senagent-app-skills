...
1. 目标 Runtime 公开信息显示新应用版本／摘要；登录与使用权正确。
2. 不挂载源目录，从制品独立创建实例、健康检查、业务调用、生产 UI 加载及重启读回。
3. 必要模型绑定与真实小样本调用；当前身份／Agent／能力归因。无网关账本读取权不得声称计费已核对。
4. 删除／覆盖／发布确认：取消零写入、重复幂等、范围变化重确认、权限撤销拒绝。
5. 执行中／待选择实例不能被强制覆盖升级；新后端 ready 失败维持旧版可用。
6. 应用自己的迁移与备份恢复；Runtime 不迁移业务库、不并行保留模板旧版，回退代码不等于回退数据。
7. 清理只限本次创建的测试实例／资源，保留脱敏证据。禁止全局清理 Runtime 或 Docker。

输出区分 packaged／staged／activated／target_verified；未调用模型、未跑浏览器、未验证架构／升级要单列。该 Skill 没有自动批准发布的脚本；通过显式 CLI 操作保留用户控制。
